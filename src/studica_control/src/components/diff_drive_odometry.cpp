...
void DiffOdometry::integrateExact(double linear, double angular) {
    // RK2 Method - Very Good for Small angular steps, near-straight motion - angular ~ 0
    if (fabs(angular) < 1e-6) integrateRungeKutta2(linear, angular);
    else {
        const double prev_heading = heading_;
        const double r = linear / angular;  // radius of the circle the robot follows.
        heading_ += angular;    
        // Displacement in an arc movement
...
void DiffOdometry::resetAccumulators() {
    linear_accumulator_  = Utils::RollingMeanAccumulator(velocity_rolling_window_size_);
