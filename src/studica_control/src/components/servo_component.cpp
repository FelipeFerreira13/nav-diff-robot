...
Servo::Servo(std::shared_ptr<VMXPi> vmx, const std::string &name, VMXChannelIndex port, studica_driver::ServoType type, int min, int max, const std::string &topic)
    ...
    servo_service_ = this->create_service<utils::srv::Int32>(
        topic + "/set_servo_angle",
        std::bind(&Servo::servo_callback, this, std::placeholders::_1, std::placeholders::_2));
...
void Servo::servo_callback(const std::shared_ptr<utils::srv::Int32::Request> request,
                                 std::shared_ptr<utils::srv::Int32::Response> response) {
    
    servo_->SetAngle(request->data);
    response->status = true;

    RCLCPP_INFO(this->get_logger(), "Set servo angle to %d degrees.", request->data);
}
