...
Titan::Titan(std::shared_ptr<VMXPi> vmx, const std::string &name, const uint8_t &canID, const uint16_t &motor_freq, const std::string &topic)
    ...
    motor_speed_srv_ = this->create_service<robot_control::srv::SetSpeed>(
        "motor/set_motor_speed", 
        std::bind(&Titan::titanVelCallback, this, std::placeholders::_1, std::placeholders::_2));

    // get_encoder_srv_ = this->create_service<studica_control::srv::GetEncoder>(
    //     "motor/get_encoder", 
    //     std::bind(&Titan::getEncCallback, this, std::placeholders::_1, std::placeholders::_2));

    service_ = this->create_service<studica_control::srv::SetData>(
        "motor/titan_cmd",
        std::bind(&Titan::cmd_callback, this, std::placeholders::_1, std::placeholders::_2));

    publisher_ = this->create_publisher<std_msgs::msg::Float32MultiArray>("motor/get_encoder", 10);
...
void Titan::titanVelCallback(std::shared_ptr<robot_control::srv::SetSpeed::Request> request, std::shared_ptr<robot_control::srv::SetSpeed::Response> response) {

    auto req = std::make_shared<studica_control::srv::SetData::Request>();
    req->initparams.speed     = request->speed;
    req->initparams.n_encoder = request->motor;

    auto res = std::make_shared<studica_control::srv::SetData::Response>();

    cmd("set_speed", req, res);

    response->status = res->success;
}

void Titan::getEncCallback(std::shared_ptr<robot_control::srv::GetEncoder::Request> request, std::shared_ptr<robot_control::srv::GetEncoder::Response> response) {
    ...
    cmd("get_encoder_count", req, res);

    response->encoder = res->enc_count;
}
...
void Titan::cmd(std::string params, std::shared_ptr<studica_control::srv::SetData::Request> request, std::shared_ptr<studica_control::srv::SetData::Response> response) {
    ...
    if (params == "enable") {
        ...
        int motor = request->initparams.n_encoder;

        RCLCPP_INFO(this->get_logger(), "Setting motor %i to %f", motor, speed);
        ...
        response->enc_count = titan_->GetEncoderCount(request->initparams.n_encoder);
    } else {
